...
- 2026-08-07: **PO AUDIT sau checkpoint — phát hiện 6 gap gate-level chưa ai ghi + đóng 2 gap.** Bằng chứng PO tự chạy trong phiên, không tin PROCESS.md cũ:
  ...
  - ✅ **Nợ #3 (nguồn data raw) = ĐÓNG MỘT PHẦN.** PO cấp link Drive `1nLtCvUsczAQ9QB-hbBDh5_w6v-joRBaX`, đã điền `data/README.md` mục 0 (chuẩn hoá bỏ tiền tố `/u/2/` vì đó là chỉ số tài khoản riêng của PO). ⚠ **Chưa đóng hoàn toàn**: probe `curl -sL` trả `<title>Google Drive: Sign-in</title>` → folder **hạn chế quyền, không public**, người ngoài mở sẽ thấy "Request access"; và session không có quyền truy cập nên **chưa verify** folder đủ 14 CSV / checksum khớp bản đĩa (126MB). → **Ask PO**: đổi share thành "Anyone with the link → Viewer" HOẶC add email từng người.
