...
| M8 | po | M0–M7 | ✅ | 538978c (merge commit, PR #1) | Makefile (fix `uv`/`pip`/`python` trần), .gitignore, data/README.md, PROCESS.md | AC1–AC8 PASS toàn bộ (chi tiết log 2026-08-07). PR #1 merged bằng **merge commit** (đúng chiến lược đã chốt, không squash) — https://github.com/lhoanghai1912/ML_Learning/pull/1. `origin/main` = `538978c`, verify 0 behind branch nguồn | **RESTRUCTURE LAKEHOUSE HOÀN TẤT, ĐÃ MERGE MAIN.** Còn nợ #1 (P2, `lookup_lag`/`validate_submission`) làm ở PR riêng v1.1 sau. |
...
- 2026-08-07: **M8 ĐÓNG — PR #1 MERGED.** `gh pr view 1` = state MERGED, mergeCommit `538978c`. `origin/main` xác nhận trỏ đúng merge commit, 0 behind `chore/restructure-lakehouse`. **Restructure lakehouse (M0–M8) hoàn tất 100%, đã lên main.** Việc còn lại thuần tech-debt, không khẩn: nợ #1 (`lookup_lag` fallback mean + `validate_submission` KeyError thô) — làm ở PR riêng v1.1, sau khi có main mới để không lẫn số đổi do fix với số đổi do restructure.
