...
| M8 | po | M0–M7 | 🟡 | 2ed1bda + (Makefile fix) | Makefile (fix `uv`/`pip`/`python` trần), .gitignore, data/README.md, PROCESS.md | **AC1 PASS**: clone sạch → `make setup && make test` = **52 passed, 1 xfailed** (Python 3.14.6, venv mới, KHÔNG có uv). **AC2 PASS**: copy raw theo đúng hướng dẫn README → `make forecast` → **BYTE-IDENTICAL** baseline. **AC3 PASS**: `git status` sau forecast chỉ thấy file đang sửa, `submission.csv` bị ignore. AC4 PASS (đã push). AC7/AC8 PASS | ❌ **CÒN LẠI: AC5 (mở PR) + merge vào main** — `origin/main` 28 behind. `gh` CLI chưa cài. Merge strategy đã chốt = **merge commit** |
